import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import random
from scipy.spatial.distance import minkowski as scipy_minkowski

# ============================
# Load Dataset
# ============================
file_path = "Lab Session Data.xlsx"
sheet_name = "marketing_campaign"

df = pd.read_excel(file_path, sheet_name=sheet_name)

# ============================================================
# A1. Identify Datatypes
# ============================================================
print("\n========== A1 ==========")
feature_types = {
    "ID":"Nominal",
    "Year_Birth":"Interval",
    "Education":"Ordinal",
    "Marital_Status":"Nominal",
    "Income":"Ratio",
    "Kidhome":"Ratio",
    "Teenhome":"Ratio",
    "Dt_Customer":"Interval",
    "Recency":"Ratio",
    "MntWines":"Ratio",
    "MntFruits":"Ratio",
    "MntMeatProducts":"Ratio",
    "MntFishProducts":"Ratio",
    "MntSweetProducts":"Ratio",
    "MntGoldProds":"Ratio",
    "NumDealsPurchases":"Ratio",
    "NumWebPurchases":"Ratio",
    "NumCatalogPurchases":"Ratio",
    "NumStorePurchases":"Ratio",
    "NumWebVisitsMonth":"Ratio",
    "AcceptedCmp3":"Nominal",
    "AcceptedCmp4":"Nominal",
    "AcceptedCmp5":"Nominal",
    "AcceptedCmp1":"Nominal",
    "AcceptedCmp2":"Nominal",
    "Complain":"Nominal",
    "Z_CostContact":"Ratio",
    "Z_Revenue":"Ratio",
    "Response":"Nominal"
}
for k,v in feature_types.items():
    print(f"{k:25} {v}")

# ============================================================
# A2. Encoding Functions
# ============================================================
print("\n========== A2 ==========")

# AI Tool Used: Claude Sonnet 5
# Implementation generated/improved with AI assistance; reviewed and integrated by the student.
def label_encode(series: pd.Series):
    """
    Perform label encoding on a pandas Series.

    Parameters
    ----------
    series : pd.Series
        Categorical column to encode.

    Returns
    -------
    encoded : pd.Series
        Series with categories replaced by integer codes.
    mapping : dict
        Mapping of original category -> integer code.
    """
    vals = sorted(series.dropna().unique())
    mapping = {v: i for i, v in enumerate(vals)}
    return series.map(mapping), mapping

# AI Tool Used: Claude Sonnet 5
# Implementation generated/improved with AI assistance; reviewed and integrated by the student.
def one_hot_encode(df: pd.DataFrame, columns: list):
    """
    Perform one-hot encoding on the given categorical columns of a DataFrame.

    Parameters
    ----------
    df : pd.DataFrame
        Input dataframe.
    columns : list
        List of column names to one-hot encode.

    Returns
    -------
    pd.DataFrame
        New dataframe with the specified columns replaced by one-hot columns.
    """
    out = df.copy()
    for col in columns:
        vals = sorted(out[col].dropna().unique())
        for v in vals:
            out[f"{col}_{v}"] = (out[col] == v).astype(int)
        out.drop(columns=col, inplace=True)
    return out

# ============================================================
# A3. Apply Encoding
# ============================================================
print("\n========== A3 ==========")

categorical=["Education","Marital_Status"]

for c in categorical:
    enc,m=label_encode(df[c])
    print(c,m)

encoded_df=one_hot_encode(df,categorical)

print("Original Shape :",df.shape)
print("Encoded Shape  :",encoded_df.shape)

# ============================================================
# Prepare Numeric Matrix
# ============================================================
numeric_df=encoded_df.select_dtypes(include=np.number).copy()

for c in ["ID"]:
    if c in numeric_df.columns:
        numeric_df.drop(columns=c,inplace=True)

numeric_df=numeric_df.fillna(numeric_df.mean(numeric_only=True))
X=numeric_df.values.astype(float)

# ============================================================
# A4. Minkowski Distance
# ============================================================
print("\n========== A4 ==========")

# AI Tool Used: Claude Sonnet 5
# Implementation generated/improved with AI assistance; reviewed and integrated by the student.
def minkowski_distance(a: np.ndarray, b: np.ndarray, p: float = 2):
    """
    Compute the generalized Minkowski distance between two vectors.

    p=1 -> Manhattan distance, p=2 -> Euclidean distance.

    Parameters
    ----------
    a, b : np.ndarray
        Input vectors of the same shape.
    p : float
        Order of the Minkowski distance (must be >= 1).

    Returns
    -------
    float
        Minkowski distance between a and b.
    """
    if p < 1:
        raise ValueError("Minkowski distance requires p >= 1.")
    return np.sum(np.abs(a - b) ** p) ** (1 / p)

# ============================================================
# A5. Distance Plot
# ============================================================
print("\n========== A5 ==========")

v1=X[0]
v2=X[1]

plist=list(range(1,11))
dist=[]

for p in plist:
    d=minkowski_distance(v1,v2,p)
    dist.append(d)
    print(f"p={p} Distance={d:.4f}")

plt.figure(figsize=(6,4))
plt.plot(plist,dist,marker="o")
plt.grid(True)
plt.xlabel("p")
plt.ylabel("Distance")
plt.title("Minkowski Distance")
plt.savefig("A5_Minkowski.png")
plt.close()

# ============================================================
# A6. Compare with scipy
# ============================================================
print("\n========== A6 ==========")

for p in plist:
    d1=minkowski_distance(v1,v2,p)
    d2=scipy_minkowski(v1,v2,p)
    print(f"p={p} Custom={d1:.6f}  Scipy={d2:.6f}")

# ============================================================
# A7. Dot Product & Norm
# ============================================================
print("\n========== A7 ==========")

# AI Tool Used: Claude Sonnet 5
# Implementation generated/improved with AI assistance; reviewed and integrated by the student.
def dot_product(a: np.ndarray, b: np.ndarray) -> float:
    """Compute the dot product of two vectors."""
    return np.sum(a * b)

# AI Tool Used: Claude Sonnet 5
# Implementation generated/improved with AI assistance; reviewed and integrated by the student.
def euclidean_norm(a: np.ndarray) -> float:
    """Compute the Euclidean (L2) norm of a vector."""
    return np.sqrt(np.sum(a * a))

print("Custom Dot :",dot_product(v1,v2))
print("NumPy Dot  :",np.dot(v1,v2))

print("Custom Norm:",euclidean_norm(v1))
print("NumPy Norm :",np.linalg.norm(v1))

# ============================================================
# A8. Mean Variance Std
# ============================================================
print("\n========== A8 ==========")

# AI Tool Used: Claude Sonnet 5
# Implementation generated/improved with AI assistance; reviewed and integrated by the student.
def my_mean(x: np.ndarray) -> float:
    """Compute the arithmetic mean of a 1-D array."""
    if len(x) == 0:
        raise ValueError("Cannot compute mean of an empty array.")
    return np.sum(x) / len(x)

# AI Tool Used: Claude Sonnet 5
# Implementation generated/improved with AI assistance; reviewed and integrated by the student.
def my_variance(x: np.ndarray) -> float:
    """Compute the population variance of a 1-D array."""
    m = my_mean(x)
    return np.sum((x - m) ** 2) / len(x)

# AI Tool Used: Claude Sonnet 5
# Implementation generated/improved with AI assistance; reviewed and integrated by the student.
def my_std(x: np.ndarray) -> float:
    """Compute the population standard deviation of a 1-D array."""
    return np.sqrt(my_variance(x))

# AI Tool Used: Claude Sonnet 5
# Implementation generated/improved with AI assistance; reviewed and integrated by the student.
def matrix_statistics(matrix: np.ndarray):
    """
    Compute per-column mean, variance, and standard deviation for a 2-D matrix.

    Parameters
    ----------
    matrix : np.ndarray
        2-D array with rows as samples and columns as features.

    Returns
    -------
    means, variances, stds : np.ndarray
        Arrays of length matrix.shape[1].
    """
    means = []
    variances = []
    stds = []
    for i in range(matrix.shape[1]):
        col = matrix[:, i]
        means.append(my_mean(col))
        variances.append(my_variance(col))
        stds.append(my_std(col))
    return np.array(means), np.array(variances), np.array(stds)

mean_custom,var_custom,std_custom=matrix_statistics(X)

# ============================================================
# A9. Compare with NumPy
# ============================================================
print("\n========== A9 ==========")

mean_np=np.mean(X,axis=0)
std_np=np.std(X,axis=0)

print("First Five Mean Values")
print(mean_custom[:5])
print(mean_np[:5])

print("First Five Std Values")
print(std_custom[:5])
print(std_np[:5])

# ============================================================
# A10. Histogram
# ============================================================
print("\n========== A10 ==========")

feature=X[:,0]

print("Mean :",my_mean(feature))
print("Variance :",my_variance(feature))

plt.figure(figsize=(6,4))
plt.hist(feature,bins=10,edgecolor="black")
plt.xlabel(numeric_df.columns[0])
plt.ylabel("Frequency")
plt.title("Histogram")
plt.savefig("A10_Histogram.png")
plt.close()

# ============================================================
# A11. K-Means
# ============================================================
print("\n========== A11 ==========")

# AI Tool Used: Claude Sonnet 5
# Implementation regenerated with AI assistance using k-means++ seeding instead
# of uniform random sampling (same function name/signature/return contract as
# Lab 03's version); reviewed and integrated by the student.
def initialize_centroids(X: np.ndarray, k: int, random_state: int = None):
    """
    Select k initial centroids from X using k-means++ seeding: the first
    centroid is chosen uniformly at random, and each subsequent centroid is
    chosen from the remaining points with probability proportional to its
    squared distance from the nearest centroid already chosen. This spreads
    out the initial centroids and tends to give faster, more consistent
    convergence than plain uniform random sampling.

    random_state: optional seed for reproducibility. If None, behaves
    non-deterministically like Lab 03's version.
    """
    rng = random.Random(random_state) if random_state is not None else random

    n_samples = len(X)
    centroid_indices = [rng.randrange(n_samples)]

    for _ in range(1, k):
        chosen = X[centroid_indices]
        # squared distance from every point to its nearest chosen centroid
        sq_dists = np.min(
            ((X[:, None, :] - chosen[None, :, :]) ** 2).sum(axis=2),
            axis=1,
        )
        total = sq_dists.sum()
        if total == 0:
            # all remaining points are duplicates of chosen centroids
            next_idx = rng.randrange(n_samples)
        else:
            probs = sq_dists / total
            next_idx = rng.choices(range(n_samples), weights=probs, k=1)[0]
        centroid_indices.append(next_idx)

    return X[centroid_indices]

# AI Tool Used: Claude Sonnet 5
# Implementation regenerated with AI assistance: replaced the per-row Python
# loop with a vectorized NumPy distance computation (same function name/
# signature/return contract as Lab 03's version); reviewed and integrated by
# the student.
def assign_clusters(X: np.ndarray, centroids: np.ndarray) -> np.ndarray:
    """
    Assign each row of X to its nearest centroid (Euclidean distance),
    computed for all points and all centroids at once via broadcasting
    instead of looping in Python over every row.
    """
    # shape: (n_samples, k) squared Euclidean distances
    sq_dists = ((X[:, None, :] - centroids[None, :, :]) ** 2).sum(axis=2)
    return np.argmin(sq_dists, axis=1)

# AI Tool Used: Claude Sonnet 5
# Implementation generated/improved with AI assistance; reviewed and integrated by the student.
def recompute_centroids(X: np.ndarray, labels: np.ndarray, k: int) -> np.ndarray:
    """
    Recompute centroid positions as the mean of points assigned to each cluster.
    If a cluster has no points assigned, a random data point is used instead
    (avoids empty-cluster degeneration).

    Parameters
    ----------
    X : np.ndarray
        Data matrix.
    labels : np.ndarray
        Current cluster assignment for each row of X.
    k : int
        Number of clusters.

    Returns
    -------
    np.ndarray
        Updated k x n_features centroid array.
    """
    centroids = []
    for i in range(k):
        pts = X[labels == i]
        if len(pts) == 0:
            centroids.append(X[random.randint(0, len(X) - 1)])
        else:
            centroid = [my_mean(pts[:, j]) for j in range(X.shape[1])]
            centroids.append(centroid)
    return np.array(centroids)

# AI Tool Used: Claude Sonnet 5
# Implementation generated/improved with AI assistance; reviewed and integrated by the student.
def kmeans(X: np.ndarray, k: int, max_iter: int = 100, random_state: int = None):
    """
    Basic K-means clustering (Lloyd's algorithm), reusing minkowski_distance
    and my_mean.

    Parameters
    ----------
    X : np.ndarray
        Data matrix.
    k : int
        Number of clusters.
    max_iter : int
        Maximum number of iterations.
    random_state : int, optional
        Seed for reproducible centroid initialization.

    Returns
    -------
    centroids : np.ndarray
        Final centroid positions.
    labels : np.ndarray
        Final cluster assignment for each row of X.
    """
    centroids = initialize_centroids(X, k, random_state=random_state)
    for _ in range(max_iter):
        labels = assign_clusters(X, centroids)
        new_centroids = recompute_centroids(X, labels, k)
        if np.allclose(centroids, new_centroids):
            break
        centroids = new_centroids
    return centroids, labels

centroids,labels=kmeans(X,3)

numeric_df["Cluster"]=labels
numeric_df.to_csv("marketing_campaign_clusters.csv",index=False)

print("Cluster Counts")
print(pd.Series(labels).value_counts().sort_index())
print("Completed Successfully.")